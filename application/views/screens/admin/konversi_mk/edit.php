...
<div class="container-fluid">
  <div class="col-sm-12">
    <div class="card">
      ...
      <form class="form theme-form" method="post" action="<?= base_url('admin/konversi_mk/update/'.$konversi_mk->id) ?>">
          <?php
                ...
                <select name="id_program_kegiatan" class="form-select js-example-basic-single">
                  ...
                  <?php
                    foreach ($kegiatan as $show) {
                      $selected = "";
                      if($konversi_mk->id_program_kegiatan == $show->id) {
                        $selected = "selected";
                      }
                ...
                <select name="kd_mk" class="form-select js-example-basic-single">
                  ...
                  <?php
                    foreach ($matakuliah as $show) {
                      $selected = "";
                      if($konversi_mk->kd_mk == $show->kd_mk) {
                        $selected = "selected";
                      }
                      echo "<option value='".$show->kd_mk."' ".$selected.">(".$show->kd_mk.") ".$show->matakuliah." </option>";
                    }
                  ?>
                </select>
              </div>
